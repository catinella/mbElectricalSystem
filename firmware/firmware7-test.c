...
//                                                                                                 |___/                       
//
// File:   firmware7-test.c
//
// Author: Silvano Catinella <catinella@yahoo.com>
//
// Description:
//	This test has been developped to ckeck for the mbesSelector module
//
//	As the previous test, it will read two input-pin's value and will copy the value in two output-pin. The following table
//	show you the pins used by the test and their association
//
//	+--------------+-------------+--------------+
...
int main() {
	...
	struct mbesSelector inA;  // MCU's pin
	struct mbesSelector inB;  // MCP23008's pin
	...
	mbesSelector_init(&inA, BUTTON, IN_A);
	...
	while (state != END) {
		...
		if (state == INITIALIZATION) {
			...
			pinStatus = mbesSelector_get(inA) ? 1 : 0;
			state = PA_WRITING;
			...
			pinStatus = mbesSelector_get(inB) ? 1 : 0;
			state = PB_WRITING;
		...
		if (mbesSelector_update(&inA) == 0 || mbesSelector_update(&inB) == 0) {
			// ERROR
			USART_writeString(PSTR("ERROR! mbesSelector_update() failed\n\n\r"), USART_FLASH);
			state = I2CBUS_ERROR;
		}
